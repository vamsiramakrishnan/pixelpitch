...
- \`PIXELPITCH_PROJECT_ID\` - active project id. Pass it as \`--project "$PIXELPITCH_PROJECT_ID"\`.
- \`PIXELPITCH_PROJECT_DIR\` - active project files directory.
- \`PIXELPITCH_DAEMON_URL\` - base URL of the local daemon.
...
\`\`\`bash
node "$PIXELPITCH_BIN" media wait <taskId> --since <nextSince>
\`\`\`
