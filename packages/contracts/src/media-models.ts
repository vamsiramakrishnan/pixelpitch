 ...
 * provider list) and the daemon (pixelpitch media generate dispatcher) consume
 ...
 *
 * Web and daemon import this shared registry through `@pixelpitch/contracts`.
 * Do not add app-local media model mirrors.
 */

import type { AudioKind, MediaAspect } from './api/projects.js';
...
export const MEDIA_PROVIDERS: MediaProvider[] = [
  ...
  },
  {
    id: 'nanobanana',
    label: 'Nano Banana',
    hint: 'Gemini API or Vertex AI image generation',
    integrated: true,
...
export function modelsForSurface(
  ...
  if (surface === 'audio') {
    ...
    return AUDIO_MODELS_BY_KIND[kind] ?? AUDIO_MODELS_BY_KIND.music;
  }
  return [];
}
