...
def plan_fanout(bottom_k: int) -> None:
    ...
    for slide_name, ssim in worst:
        ...
        tasks.append({
            ...
            "html_path": str((BENCH / "corpus" / slide_name).relative_to(ROOT)),
...
- **Slide:** `{(BENCH / "corpus" / slide_name).relative_to(ROOT)}`
...
1. Run baseline:
   ```
   python {BENCH.relative_to(ROOT)}/scripts/run_corpus.py --score
   ```
   Confirm `{slide_name}` SSIM matches {baseline_ssim:.3f}.

2. Render the heatmap:
   ```
   python {BENCH.relative_to(ROOT)}/scripts/render_heatmap.py
   ```
   Open `{BENCH.relative_to(ROOT)}/corpus/heatmap/{Path(slide_name).stem}.png` — the red
