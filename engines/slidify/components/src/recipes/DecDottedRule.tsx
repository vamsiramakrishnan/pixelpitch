...
export default function DecDottedRule(props: DecDottedRuleProps): ReactNode {
  ...
  return (
    <div data-recipe-id="dec.dotted-rule" data-recipe-version="1.0.0">
      <DecorationLineStroke {...({ bbox: props.bbox, orientation: props.orientation ?? "h", color: props.color ?? tokens.palette("ruler"), dash: 'dotted' } as unknown as ComponentProps<typeof DecorationLineStroke>)} />
    </div>
  );
}
...
export function decDottedRuleToIR(
  ...
  const primitiveArgs = { bbox: props.bbox, orientation: props.orientation ?? "h", color: props.color ?? tokens.palette("ruler"), dash: 'dotted' } as unknown as Parameters<typeof decorationLineStrokeToIR>[0];
