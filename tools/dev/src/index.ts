...
import { createSidecarLaunchEnv, requestJsonIpc } from "@pixelpitch/sidecar";
import {
  ...
  try {
    ...
    return await spawnSidecarRuntime({
      ...
      env: {
        ...
        ...(options.parentPid == null ? {} : { [TOOLS_DEV_PARENT_PID_ENV]: String(options.parentPid) }),
        ...(options.prod === true
          ? { NODE_ENV: "production", PIXELPITCH_WEB_OUTPUT_MODE: "server", PIXELPITCH_WEB_PROD: "1" }
          : {}),
      },
      logHandle,
    });
...
async function runForeground(config: ToolDevConfig, appName: string | undefined, options: CliOptions) {
  ...
  await new Promise<void>((resolveDone) => {
    const shutdown = () => {
      ...
      process.stderr.write("\nStopping Pixelpitch dev server...\n");
