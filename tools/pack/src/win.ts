...
import {
  ...
  stopProcesses,
} from "@pixelpitch/platform";

import type { ToolPackConfig } from "./config.js";
import { winResources } from "./resources.js";

const execFileAsync = promisify(execFile);
const PRODUCT_NAME = "Pixelpitch";
const DESKTOP_LOG_ECHO_ENV = "PIXELPITCH_DESKTOP_LOG_ECHO";
...
const INTERNAL_PACKAGES = [
  { directory: "packages/contracts", name: "@pixelpitch/contracts" },
  { directory: "packages/sidecar-proto", name: "@pixelpitch/sidecar-proto" },
  { directory: "packages/sidecar", name: "@pixelpitch/sidecar" },
  { directory: "packages/platform", name: "@pixelpitch/platform" },
  { directory: "apps/daemon", name: "@pixelpitch/daemon" },
  { directory: "apps/web", name: "@pixelpitch/web" },
  { directory: "apps/desktop", name: "@pixelpitch/desktop" },
  { directory: "apps/packaged", name: "@pixelpitch/packaged" },
] as const;
...
function resolveWinPaths(config: ToolPackConfig): WinPaths {
  ...
  return {
    ...
    packagedConfigPath: join(namespaceRoot, "pixelpitch-config.json"),
    resourceRoot: join(namespaceRoot, "resources", "pixelpitch"),
...
LangString PIXELPITCH_REMOVE_LOCAL_DATA_TITLE 1033 "Remove local data"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_TITLE 2052 "删除本地数据"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_TITLE 1028 "刪除本機資料"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_TITLE 1046 "Remover dados locais"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_TITLE 1049 "Удалить локальные данные"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_TITLE 1065 "حذف داده‌های محلی"

LangString PIXELPITCH_REMOVE_LOCAL_DATA_HINT 1033 "Choose whether the uninstaller should remove Pixelpitch data stored on this computer."
LangString PIXELPITCH_REMOVE_LOCAL_DATA_HINT 2052 "请选择卸载程序是否删除此电脑上保存的 Pixelpitch 数据。"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_HINT 1028 "請選擇解除安裝程式是否刪除此電腦上儲存的 Pixelpitch 資料。"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_HINT 1046 "Escolha se o desinstalador deve remover os dados do Pixelpitch armazenados neste computador."
LangString PIXELPITCH_REMOVE_LOCAL_DATA_HINT 1049 "Выберите, должен ли деинсталлятор удалить данные Pixelpitch, сохраненные на этом компьютере."
LangString PIXELPITCH_REMOVE_LOCAL_DATA_HINT 1065 "انتخاب کنید که حذف‌کننده داده‌های Pixelpitch ذخیره‌شده در این رایانه را حذف کند یا نه."

LangString PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX 1033 "Remove local Pixelpitch data:"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX 2052 "删除本地 Pixelpitch 数据："
LangString PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX 1028 "刪除本機 Pixelpitch 資料："
LangString PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX 1046 "Remover dados locais do Pixelpitch:"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX 1049 "Удалить локальные данные Pixelpitch:"
LangString PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX 1065 "حذف داده‌های محلی Pixelpitch:"

!macro customUnWelcomePage
  !insertmacro MUI_UNPAGE_WELCOME
  UninstPage custom un.PixelpitchLocalDataPage un.PixelpitchLocalDataPageLeave
!macroend

Function un.PixelpitchLocalDataPage
  ...
  \${NSD_CreateLabel} 0 0 100% 24u "$(PIXELPITCH_REMOVE_LOCAL_DATA_HINT)"
  Pop $0
  \${NSD_CreateCheckbox} 0 34u 100% 36u "$(PIXELPITCH_REMOVE_LOCAL_DATA_CHECKBOX) $odLocalDataRoot"
  Pop $odRemoveLocalDataCheckbox
  \${NSD_Check} $odRemoveLocalDataCheckbox
  nsDialogs::Show
FunctionEnd

Function un.PixelpitchLocalDataPageLeave
...
!macro customUnInstall
  ...
  \${If} $odRemoveLocalData != "0"
    DetailPrint "Removing local Pixelpitch data: $odLocalDataRoot"
...
async function writeAssembledApp(config: ToolPackConfig, paths: WinPaths, packedTarballs: PackedTarballInfo[]): Promise<void> {
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        dependencies,
        description: "Pixelpitch packaged runtime",
        main: "./main.cjs",
        name: "pixelpitch-packaged-app",
        private: true,
    ...
    `${JSON.stringify(
      {
        ...
        nodeCommandRelative: join("pixelpitch", "bin", "node.exe"),
...
async function runElectronBuilder(config: ToolPackConfig, paths: WinPaths): Promise<void> {
  ...
  const builderConfig = {
    ...
    extraResources: [
      { from: paths.resourceRoot, to: "pixelpitch" },
      { from: paths.packagedConfigPath, to: "pixelpitch-config.json" },
    ],
    ...
    productName: PRODUCT_NAME,
    publish: [{ provider: "generic", url: "https://updates.invalid/pixelpitch" }],
    win: {
