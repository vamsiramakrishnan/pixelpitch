...
Local packaging control plane for Pixelpitch.
...
Release artifacts keep the canonical `Pixelpitch.app` bundle shape; local `tools-pack install` copies it as
`Pixelpitch.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
