...
- Real connector mode: use the injected `"$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN"`
  wrapper commands described in `references/connectors.md` to execute connector
  tools and register/update a Live Artifact. Do not call authenticated tool
  endpoints from browser code and never expose `PIXELPITCH_TOOL_TOKEN` inside
  the artifact. Until Pixelpitch exposes a scoped browser poll route, keep the
  in-page refresh behavior on the seeded mock so the preview never appears
  broken.
...
- `index.html` — single self-contained file, no external CSS / JS
  imports beyond a system font stack and a single Pixelpitch `<live-counter>`
  custom element.
