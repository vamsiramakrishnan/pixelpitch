...
name: live-artifact
description: |
  Create refreshable, auditable Pixelpitch artifacts backed by connector or local data.
...
Create a Pixelpitch live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
1. **Resolve scope and data source without blocking on connected connectors**
   ...
   - If connector data is needed, first list connectors with `"$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN" tools connectors list --format compact`. If the named connector is present with `status: "connected"`, choose an appropriate read-only `auto` tool from its catalog and execute it through the connector wrapper.
...
5. **Register or update through daemon wrappers**
   - Use the Pixelpitch daemon wrapper commands via `"$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN"` instead of raw `curl`, bare `node`, or bare `pixelpitch`:

     ```bash
     "$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN" tools live-artifacts create --input artifact.json
     "$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN" tools live-artifacts list --format compact
     "$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN" tools live-artifacts update --artifact-id "$ARTIFACT_ID" --input artifact.json
     ```

   - The wrapper reads injected `PIXELPITCH_NODE_BIN`, `PIXELPITCH_BIN`, `PIXELPITCH_DAEMON_URL`, and `PIXELPITCH_TOOL_TOKEN`; do not print, persist, or override token values.
...
6. **Use connector wrappers for connector data**
   ...
   - Execute a read-only connector tool with a JSON object input file:
     ...
     ```bash
     "$PIXELPITCH_NODE_BIN" "$PIXELPITCH_BIN" tools connectors execute --connector "$CONNECTOR_ID" --tool "$TOOL_NAME" --input input.json
     ```
