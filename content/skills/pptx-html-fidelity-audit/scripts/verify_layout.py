...
def is_full_slide_raster(shape, top: float, left: float, right: float, bottom: float,
                         canvas_w: float, canvas_h: float) -> bool:
    """Slidify may emit a full-slide raster fallback behind/alongside native
    shapes. It should still be checked against the canvas bounds above, but it
    is not a content box and should not fail the footer-rail invariant."""
    if shape.shape_type != MSO_SHAPE_TYPE.PICTURE:
        return False
    return (
        abs(top) <= EPS_IN and
        abs(left) <= EPS_IN and
        abs(right - canvas_w) <= EPS_IN and
        abs(bottom - canvas_h) <= EPS_IN
    )


def is_footer_rule(name: str, top: float, bottom: float, content_max_y: float,
                   footer_zone_top: float) -> bool:
    n = (name or "").lower()
    height = bottom - top
    if not ("connector" in n or "line" in n or "rule" in n):
        return False
    if height <= 0.15 and top >= content_max_y - 0.10:
        return True
    return bottom <= footer_zone_top + 0.10
...
def verify(path: Path, content_max_y: float, canvas_w: float, canvas_h: float,
    ...
    for i, slide in enumerate(prs.slides, 1):
        for shape in slide.shapes:
            ...
            # (b) pinned at-or-below the footer zone threshold, or
            # (c) a full-canvas raster fallback generated by Slidify, or
            # (d) a thin footer separator rule just above the footer zone.
            if is_footer_by_name(name) or top >= footer_zone_top - EPS_IN:
                continue
            if is_full_slide_raster(shape, top, left, right, bottom, canvas_w, canvas_h):
                continue
            if is_footer_rule(name, top, bottom, content_max_y, footer_zone_top):
                continue
