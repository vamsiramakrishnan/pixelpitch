...
// dev-all launcher overrides PIXELPITCH_PORT after probing for a free port; we read
// the same env so /api, /artifacts, and /frames always reach the right
// daemon instance during `next dev`.
const DAEMON_PORT = Number(process.env.PIXELPITCH_PORT) || 17456;
...
// server runtime with PIXELPITCH_WEB_OUTPUT_MODE=server; in that mode the web sidecar
// owns the Next.js SSR server and proxies daemon routes at runtime.
const isProd = process.env.NODE_ENV !== 'development';
const isServerOutput = process.env.PIXELPITCH_WEB_OUTPUT_MODE === 'server';
...
function resolveDistDir(defaultValue: string) {
  if (process.env.PIXELPITCH_WEB_PROD === '1') return defaultValue;
  const configured = process.env.PIXELPITCH_WEB_DIST_DIR;
...
function resolveDevTsconfigPath() {
  const configured = process.env.PIXELPITCH_WEB_TSCONFIG_PATH;
