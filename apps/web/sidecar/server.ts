...
} from "@pixelpitch/sidecar";

const HOST = process.env.PIXELPITCH_HOST || "127.0.0.1";
if (process.env.PIXELPITCH_HOST != null && !/^[a-zA-Z0-9._\-:[\]@]+$/.test(process.env.PIXELPITCH_HOST)) {
  throw new Error(`PIXELPITCH_HOST contains invalid characters: ${process.env.PIXELPITCH_HOST}`);
}
...
function resolveWebRoot(): string {
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    try {
      ...
      if (packageJson.name === "@pixelpitch/web") return current;
    } catch {
  ...
  throw new Error("failed to resolve @pixelpitch/web package root");
}
...
export async function startWebSidecar(runtime: SidecarRuntimeContext<SidecarStamp>): Promise<WebSidecarHandle> {
  ...
  const app = createNextServer({ dev: process.env.PIXELPITCH_WEB_PROD !== "1" && runtime.mode === "dev", dir });
