...
describe('resolveDaemonProxyTarget', () => {
  it('proxies allowlisted relative paths to the daemon origin', () => {
    const target = resolveDaemonProxyTarget('http://127.0.0.1:17456', '/api/projects?limit=10');

    expect(target?.href).toBe('http://127.0.0.1:17456/api/projects?limit=10');
  });
  ...
  it('does not let absolute request URLs replace the daemon origin', () => {
    ...
    expect(target?.href).toBe('http://127.0.0.1:17456/api/latest/meta-data?token=1');
  });

  it('rejects non-daemon paths', () => {
    expect(resolveDaemonProxyTarget('http://127.0.0.1:17456', '/settings')).toBeNull();
  });
});
