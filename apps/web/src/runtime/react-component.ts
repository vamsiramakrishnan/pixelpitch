    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__PixelpitchComponent ||
...
window.__PixelpitchComponent = window.__PixelpitchComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'PixelpitchComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'PixelpitchComponent';
    return 'const PixelpitchComponent = ';
  });
  code = code.replace(
