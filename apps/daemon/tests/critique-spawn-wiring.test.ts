...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pixelpitch-spawn-wiring-test-'));
  db = freshDb();
});
afterEach(async () => {
  db.close();
...
// ---------------------------------------------------------------------------
// Config gate: PIXELPITCH_CRITIQUE_ENABLED=false (legacy path unchanged)
// ---------------------------------------------------------------------------
...
describe('spawn wiring - cfg.enabled=false (M0 default)', () => {
  ...
  it('loadCritiqueConfigFromEnv with PIXELPITCH_CRITIQUE_ENABLED=false returns enabled=false', () => {
...
// ---------------------------------------------------------------------------
// Config gate: PIXELPITCH_CRITIQUE_ENABLED=true (orchestrator path)
// ---------------------------------------------------------------------------

describe('spawn wiring - cfg.enabled=true (orchestrator path)', () => {
  it('with PIXELPITCH_CRITIQUE_ENABLED=true, runOrchestrator is invoked with the spawn stdout', async () => {
