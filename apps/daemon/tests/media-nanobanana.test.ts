...
describe('nano-banana media generation', () => {
  ...
  const originalNanoBananaKey = process.env.PIXELPITCH_NANOBANANA_API_KEY;
  const originalGoogleKey = process.env.GOOGLE_API_KEY;
  const originalGeminiKey = process.env.GEMINI_API_KEY;

  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), 'pixelpitch-nanobanana-'));
    projectRoot = path.join(root, 'project-root');
    projectsRoot = path.join(projectRoot, '.pixelpitch', 'projects');
    await mkdir(projectsRoot, { recursive: true });
    process.env.PIXELPITCH_NANOBANANA_API_KEY = 'nano-test-key';
  ...
  afterEach(async () => {
    ...
    if (originalNanoBananaKey == null) {
      delete process.env.PIXELPITCH_NANOBANANA_API_KEY;
    } else {
      process.env.PIXELPITCH_NANOBANANA_API_KEY = originalNanoBananaKey;
    }
  ...
  async function writeConfig(data: unknown) {
    const file = path.join(projectRoot, '.pixelpitch', 'media-config.json');
  ...
  it('supports Vertex AI publisher model endpoints', async () => {
    await writeConfig({
      providers: {
        nanobanana: {
          baseUrl: 'https://us-central1-aiplatform.googleapis.com/v1/projects/pixelpitch-test/locations/us-central1',
          model: 'gemini-3.1-flash-image-preview',
        },
      },
    });
    ...
    const fetchMock = vi.fn(async (input: unknown, init?: RequestInit) => {
      expect(String(input)).toBe(
        'https://us-central1-aiplatform.googleapis.com/v1/projects/pixelpitch-test/locations/us-central1/publishers/google/models/gemini-3.1-flash-image-preview:generateContent',
      );
      expect(init?.headers).toMatchObject({
        authorization: 'Bearer nano-test-key',
        'content-type': 'application/json',
    ...
    const result = await generateMedia({
      ...
      surface: 'image',
      model: 'gemini-3.1-flash-image-preview',
      prompt: 'A product render on a Vertex AI endpoint',
      aspect: '4:3',
      output: 'vertex.png',
    });

    expect(result.providerId).toBe('nanobanana');
    expect(result.providerNote).toContain('vertex');
    expect(result.name).toBe('vertex.png');
  });
