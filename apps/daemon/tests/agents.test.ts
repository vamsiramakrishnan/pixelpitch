...
const originalAgentHome = process.env.PIXELPITCH_AGENT_HOME;

afterEach(() => {
  if (originalDisablePlugins == null) {
    delete process.env.PIXELPITCH_CODEX_DISABLE_PLUGINS;
  } else {
    process.env.PIXELPITCH_CODEX_DISABLE_PLUGINS = originalDisablePlugins;
  }
  ...
  if (originalAgentHome == null) {
    delete process.env.PIXELPITCH_AGENT_HOME;
  } else {
    process.env.PIXELPITCH_AGENT_HOME = originalAgentHome;
  }
});

test('codex args disable plugins when PIXELPITCH_CODEX_DISABLE_PLUGINS is 1', () => {
...
test('codex args keep plugins enabled when PIXELPITCH_CODEX_DISABLE_PLUGINS is unset', () => {
  delete process.env.PIXELPITCH_CODEX_DISABLE_PLUGINS;
...
test('codex args keep plugins enabled when PIXELPITCH_CODEX_DISABLE_PLUGINS is not 1', () => {
  process.env.PIXELPITCH_CODEX_DISABLE_PLUGINS = 'true';
...
fsTest('resolveAgentExecutable prefers def.bin over fallbackBins when bin is on PATH', () => {
  ...
  try {
    ...
    process.env.PIXELPITCH_AGENT_HOME = dir;
...
fsTest('resolveAgentExecutable searches mise node bins when PATH is minimal', () => {
  ...
  try {
    ...
    process.env.PIXELPITCH_AGENT_HOME = home;
