...
async function makeProjectsRoot() {
  const root = await mkdtemp(path.join(tmpdir(), 'pixelpitch-live-artifacts-'));
  tempRoots.push(root);
...
describe('live artifact store layout', () => {
  ...
  it('maps document refresh data paths directly even when a legacy transform is present', () => {
    const document: any = {
      ...
      dataJson: {
        repository: {
          fullName: 'nexu-io/open',
          url: 'https://github.com/nexu-io/open',
          starCount: 100,
      ...
      sourceJson: {
        ...
        input: { url: 'https://api.github.com/repos/nexu-io/open' },
        outputMapping: {
          dataPaths: [
    ...
    const candidate = buildLiveArtifactRefreshCandidate({
      ...
      documentOutput: {
        output: {
          ...
          full_name: 'nexu-io/open',
          html_url: 'https://github.com/nexu-io/open',
