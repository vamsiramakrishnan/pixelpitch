...
<html lang="en">
  <head>
    ...
    <title>${connectorLabelHtml} connected · Pixelpitch</title>
    <style>
      :root {
        --bg: #faf9f7;
        --bg-panel: #ffffff;
  ...
  <body>
    <main aria-labelledby="callback-title">
      <div class="chrome" aria-label="Pixelpitch">
        <span class="brand-mark" aria-hidden="true">Pixelpitch</span>
        <span class="brand-title">Pixelpitch</span>
      </div>
      <section class="content">
        ...
        <div>
          ...
          <p>Your connector is ready to use in Pixelpitch.</p>
        </div>
    ...
    <script>
      (() => {
        ...
        const message = { type: 'open:connector-connected', connectorId, connectorLabel };
        try {
          if (window.opener && !window.opener.closed) {
            window.opener.postMessage(message, '*');
            window.setTimeout(() => window.close(), 900);
          } else {
            document.getElementById('auto-close-hint').textContent = 'You can close this tab and return to Pixelpitch.';
          }
        } catch {
          document.getElementById('auto-close-hint').textContent = 'You can close this tab and return to Pixelpitch.';
        }
