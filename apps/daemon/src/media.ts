...
//   `pixelpitch media generate --surface … --model … --output … --prompt …`
...
//
// The fallback stub handlers are gated behind PIXELPITCH_MEDIA_ALLOW_STUBS=1; in
...
// behind PIXELPITCH_MEDIA_ALLOW_STUBS=1 and otherwise return a 503 (mapped from
...
function stubsAllowed() {
  const v = process.env.PIXELPITCH_MEDIA_ALLOW_STUBS;
  return v === '1' || v === 'true';
}
 ...
 *
 * @param {Object} args
 * @param {string} args.projectRoot   - Repo root (.pixelpitch/ lives directly under).
 * @param {string} args.projectsRoot  - Absolute path to <repo>/.pixelpitch/projects.
...
export async function generateMedia(args) {
  ...
  try {
    if (def.provider === 'openai' && surface === 'image') {
      ...
      // stub fallback behind PIXELPITCH_MEDIA_ALLOW_STUBS so release builds don't
...
async function renderVolcengineVideo(ctx, credentials, onProgress) {
  ...
  const configuredMaxMs = Number(process.env.PIXELPITCH_VOLCENGINE_VIDEO_MAX_POLL_MS);
  const maxMs =
...
async function renderGrokVideo(ctx, credentials, onProgress) {
  ...
  if (!videoUrl && requestId) {
    const startedAt = Date.now();
    const configuredMaxMs = Number(process.env.PIXELPITCH_GROK_VIDEO_MAX_POLL_MS);
    const maxMs =
      Number.isFinite(configuredMaxMs) && configuredMaxMs >= 60_000
        ? configuredMaxMs
        : 8 * 60 * 1000;
    ...
    if (!videoUrl) {
      ...
      throw new Error(
        ...
        + `If your jobs legitimately need longer, raise PIXELPITCH_GROK_VIDEO_MAX_POLL_MS.`,
      );
    }
  }
...
async function renderMinimaxTTS(ctx, credentials) {
  if (!credentials.apiKey) {
    throw new Error(
      'no MiniMax API key — configure it in Settings or set PIXELPITCH_MINIMAX_API_KEY',
    );
  }
...
async function renderFishAudioTTS(ctx, credentials) {
  if (!credentials.apiKey) {
    throw new Error(
      'no FishAudio API key — configure it in Settings or set PIXELPITCH_FISHAUDIO_API_KEY',
    );
  }
...
async function renderHyperFramesViaCli(ctx, projectDir, onProgress) {
  ...
  if (typeof compRel !== 'string' || !compRel.trim()) {
    throw new Error(
      'hyperframes-html requires --composition-dir <project-relative-path> ' +
        ...
        '$PIXELPITCH_PROJECT_DIR/.hyperframes-cache/<id>/ and pass that path here.',
    );
  }
  ...
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'pixelpitch-hf-'));
