 ...
 * binary). Instead it shells out to a single command — `pixelpitch media
...
shell command — \`pixelpitch media generate\` — is HOW you actually produce bytes.
...
- \`PIXELPITCH_BIN\`         — absolute path to the \`od\` CLI script. Run with \`node "$PIXELPITCH_BIN" …\`.
- \`PIXELPITCH_PROJECT_ID\`  — the active project's id. Pass it as \`--project "$PIXELPITCH_PROJECT_ID"\`.
- \`PIXELPITCH_PROJECT_DIR\` — the project's files folder (your cwd). Generated files land here.
- \`PIXELPITCH_DAEMON_URL\`  — base URL of the local daemon, e.g. \`http://127.0.0.1:17456\`.
...
TODO (post-v1): teach \`pixelpitch media generate\` to auto-spawn a transient
...
\`\`\`bash
node "$PIXELPITCH_BIN" media generate \\
  --project "$PIXELPITCH_PROJECT_ID" \\
...
For media projects, \`node "$PIXELPITCH_BIN" media generate …\` is the **only**
...
before the first generate attempt. Treat \`PIXELPITCH_BIN\`, \`PIXELPITCH_PROJECT_ID\`,
and \`PIXELPITCH_DAEMON_URL\` as the source of truth and try the dispatcher
first.
...
COMP="$PIXELPITCH_PROJECT_DIR/$COMP_REL"
...
\`pixelpitch media generate\` no longer blocks for the full render. It dispatches
the task daemon-side and returns within ~1s with a \`{taskId}\`. You then
drive the render to completion by calling \`pixelpitch media wait <taskId>\` in
...
- \`exit 2\` — still **running**. Final stdout line is
  ...
  \`pixelpitch media wait <taskId> --since <n>\` to continue from where you left
...
\`\`\`bash
out=$(node "$PIXELPITCH_BIN" media generate --surface video --model … --image …)
ec=$?
...
while [ "$ec" -eq 2 ] && [ -n "$task_id" ]; do
  out=$(node "$PIXELPITCH_BIN" media wait "$task_id" --since "$since")
  ec=$?
...
- **video**:   ${VIDEO_IDS}
  ...
  \`--image <project-relative-path>\` to \`pixelpitch media generate\`. The
...
2. **One discovery turn before generating.** Even with metadata defaults
   ...
   message first, and do not call \`pixelpitch media generate\` (that path is
   intentionally rejected for this model).
3. **Generate by shell, narrate in chat.** When you actually invoke
   \`pixelpitch media generate\`, do it inside a clearly-labelled tool call. After
...
4. **Iterate by re-running.** To revise, call \`pixelpitch media generate\` again
...
1. **HTTP status.** When stubs are disabled (the default release-build
   ...
   error message. Set \`PIXELPITCH_MEDIA_ALLOW_STUBS=1\` to write a labelled
   placeholder instead.
2. **Exit code.** \`pixelpitch media generate\` and \`pixelpitch media wait\` exit:
