...
//
// `pixelpitch mcp` - stdio MCP server that proxies read-only tool calls to the
// running daemon's HTTP API. Lets a coding agent in a *different* repo
// (Claude Code, Cursor, Zed) pull files from a local Pixelpitch
// project without the export-zip-import dance.
//
// The server itself holds no state and never touches the filesystem;
// every tool resolves to a fetch() against `PIXELPITCH_DAEMON_URL`. Spawn the
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no Pixelpitch activity).',
} as const;

const TOOL_DEFS = [
  {
    name: 'list_projects',
    description: 'List every Pixelpitch project on this daemon.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Pixelpitch projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in Pixelpitch right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with Pixelpitch (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no Pixelpitch activity.",
        },
        include: {
          type: 'string',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no Pixelpitch activity.',
        },
        offset: {
          type: 'number',
  ...
  // MCP tools. Skills are recipes that Pixelpitch itself uses to
  ...
  // user can opt into via the resource URIs (pixelpitch://design-systems/...)
  // when they actually want them, instead of paying tool-description
  // tokens on every turn.
];

export async function runMcpStdio({ daemonUrl }) {
  const baseUrl = String(daemonUrl || process.env.PIXELPITCH_DAEMON_URL || 'http://127.0.0.1:17456').replace(/\/$/, '');
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        'Pixelpitch is a local-first design workspace. The user typically',
        'has Pixelpitch running on their machine; each project contains a rendered',
        ...
        'default to the project the user has open in Pixelpitch right now; get_file',
        ...
        '    [pixelpitch:file-window ...] marker when the file is longer; page',
        ...
        'pixelpitch://design-systems/<id>/DESIGN.md when you need the brand spec',
        'for a design (palette, typography, voice). Skills are similarly',
        'available at pixelpitch://skills/<id>/SKILL.md but are mostly relevant',
        'when the user asks about how a particular artifact was generated.',
        '',
        'When extending a Pixelpitch design in another codebase, pull',
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => {
    ...
    const resources = [
      {
        uri: 'pixelpitch://focus/active',
        name: 'Active Pixelpitch context',
        description: 'The project/file the user has open in Pixelpitch right now.',
        mimeType: 'application/json',
      },
    ];
    for (const s of skillsData?.skills || []) {
      resources.push({
        uri: `pixelpitch://skills/${encodeURIComponent(s.id)}/SKILL.md`,
    ...
    for (const d of dsData?.designSystems || []) {
      resources.push({
        uri: `pixelpitch://design-systems/${encodeURIComponent(d.id)}/DESIGN.md`,
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
    ...
    if (uri === 'pixelpitch://focus/active') {
    ...
    const m = String(uri || '').match(/^pixelpitch:\/\/(skills|design-systems)\/([^/]+)\/(.+)$/);
    if (!m) {
      ...
      switch (name) {
        ...
        case 'get_active_context': {
          ...
          if (!data || data.active === false) {
            return ok({
              active: false,
              hint: 'Pixelpitch has no active project right now. The active context expires about 5 minutes after the last user interaction with Pixelpitch, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
            });
          }
          return ok(data);
        }
...
// renamed in the Pixelpitch UI shows up within a few seconds.
...
// open in Pixelpitch. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(baseUrl, arg) {
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and Pixelpitch has no active project. The active context expires about 5 minutes after the last user interaction with Pixelpitch - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
async function getFile(baseUrl, project, relPath, active, resolved?, offset = 0, limit = 2000) {
  ...
  if (!isTextualMime(mime)) {
    return errorResult(
      `file at "${relPath}" has mime "${mime}"; binary content is not yet supported by pixelpitch mcp. Use list_files to inspect its metadata.`,
    );
  }
  ...
  if (resolved && (resolved.source === 'slug' || resolved.source === 'substring')) {
    extra.push(`[pixelpitch:resolved-project id="${resolved.id}" name="${resolved.name}" via="${resolved.source}"]`);
  }
  if (truncated || start > 0) {
    ...
    extra.push(
      `[pixelpitch:file-window offset=${start} returnedLines=${returnedLines} totalLines=${totalLines}${next}]`,
    );
  }
  return {
    content: [
...
function formatActiveEchoLine(active, resolvedPath) {
  ...
  const note = `[pixelpitch:active-context project="${proj}" file="${resolvedPath}"]`;
...
function formatError(err, daemonUrl) {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the Pixelpitch daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
