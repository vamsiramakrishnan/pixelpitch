...
// Keep this structurally aligned with `@pixelpitch/contracts` AppVersionInfo.
...
export function resolveAppVersionInfo({
  ...
  const channel = cleanString(env.PIXELPITCH_RELEASE_CHANNEL)
    ?? cleanString(env.PIXELPITCH_APP_CHANNEL)
    ?? prereleaseChannel
