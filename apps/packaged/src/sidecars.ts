...
import { createProcessStampArgs, stopProcesses, waitForProcessExit } from "@pixelpitch/platform";
...
type PackagedDaemonManagedPathEnv = {
  PIXELPITCH_DATA_DIR: string;
  PIXELPITCH_RESOURCE_ROOT: string;
};
...
function createPackagedDaemonManagedPathEnv(
  ...
  return {
    PIXELPITCH_DATA_DIR: paths.dataRoot,
    PIXELPITCH_RESOURCE_ROOT: paths.resourceRoot,
  };
}
...
async function spawnSidecarChild(options: {
  ...
  const child = spawn(
    command,
    [options.entryPath, ...createProcessStampArgs(stamp, PIXELPITCH_SIDECAR_CONTRACT)],
    {
      cwd: process.cwd(),
      env: childEnv,
...
export async function startPackagedSidecars(
  ...
  try {
    const daemon = await spawnSidecarChild({
      app: APP_KEYS.DAEMON,
      entryPath: resolveSidecarEntry("@pixelpitch/daemon", "sidecar"),
      env: {
    ...
    const web = await spawnSidecarChild({
      app: APP_KEYS.WEB,
      entryPath: resolveSidecarEntry("@pixelpitch/web", "sidecar"),
      env: {
