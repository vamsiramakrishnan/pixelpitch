...
import { SIDECAR_DEFAULTS, normalizeNamespace } from "@pixelpitch/sidecar-proto";

export const PACKAGED_CONFIG_PATH_ENV = "PIXELPITCH_PACKAGED_CONFIG_PATH";
export const PACKAGED_NAMESPACE_ENV = "PIXELPITCH_PACKAGED_NAMESPACE";
...
function resolveDefaultConfigPath(): string {
  return join(process.resourcesPath, "pixelpitch-config.json");
}
...
async function readRawPackagedConfig(): Promise<RawPackagedConfig> {
  ...
  return (
    ...
    (await readJsonIfExists(join(app.getAppPath(), "pixelpitch-config.json"))) ??
    {}
  );
}
...
export async function readPackagedConfig(): Promise<PackagedConfig> {
  ...
  const resourceRoot = resolveOptionalPath(raw.resourceRoot) ?? join(process.resourcesPath, "pixelpitch");
  const relativeNodeCommand =
    raw.nodeCommandRelative == null || raw.nodeCommandRelative.length === 0
      ? join("pixelpitch", "bin", "node")
