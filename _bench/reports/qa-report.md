...
- [major · FONT-SUB] Cover display type resolves to non-portable families.
  ...
  **Source HTML**
  - `_bench/decks/llm-corpus/16-mag-cover.html`: title/kicker use `font-family: 'Bebas Neue'` and serif deck text uses `Playfair Display`.
...
- [major · LAYOUT-DRIFT] Right-column editorial stack drifts from authored geometry.
  ...
  **Source HTML**
  - `_bench/decks/llm-corpus/17-mag-spread.html` has one drop-cap element and one body flow anchor.
  ...
  **Next investigation step (patch-ready instrumentation)**
  ...
  ```
  Run:
  `slidify convert _bench/decks/llm-corpus/17-mag-spread.html /tmp/x.pptx` and diff top 4 drift ids.
...
- [major · FONT-SUB] Brutalist headline stack emits Helvetica Neue and depends on host substitution.
  ...
  **Source HTML**
  - `_bench/decks/llm-corpus/18-bru-manifesto.html`: `font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-weight: 900;`.
...
- [major · FONT-SUB] Numeric wall uses unresolved Helvetica Neue at multiple weights.
  ...
  **Source HTML**
  - `_bench/decks/llm-corpus/19-bru-stat-wall.html` uses same Helvetica Neue stack for stats.
...
- [major · COLOR-SHIFT] Duotone treatment is emitted as flat shape stack (no gradient stops).
  ...
  **Source HTML**
  - `_bench/decks/llm-corpus/22-duo-photo.html` uses layered translucent overlays/duotone effect.
...
- [major · COLOR-SHIFT] Plate duotone reduced to simplified shape-color approximation.
  ...
  **Source HTML**
  - `_bench/decks/atlas-vol-iii/04-duotone-plate.html` uses layered duotone treatment with opacity blending.
...
- [major · COLOR-SHIFT] Caption overlay alpha appears flattened.
  ...
  **Source HTML**
  - `_bench/decks/atlas-vol-iii/08-cinemagraph.html` caption uses rgba backdrop over animated plate.
