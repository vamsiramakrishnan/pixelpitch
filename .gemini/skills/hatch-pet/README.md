...
checked into the Pixelpitch repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Pixelpitch agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Pixelpitch and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any Pixelpitch-specific notes
   inside `SKILL.md`'s `> **Pixelpitch integration.**` blockquote.
...
flow (including how Pixelpitch behaves when Codex is not installed).
