...
//      and an "Pixelpitch integration" section near the top of
...
// frontmatter shim and the "Pixelpitch integration" section.
...
async function main() {
  ...
  try {
    ...
    if (process.env.PIXELPITCH_KEEP_HF_SYNC_TMP) {
      console.log(`[sync] PIXELPITCH_KEEP_HF_SYNC_TMP set — leaving ${tmpRoot}`);
    } else {
