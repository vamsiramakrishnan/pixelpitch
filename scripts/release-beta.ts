...
const BETA_TAG = "pixelpitch-beta";
const stableVersionPattern = /^(\d+)\.(\d+)\.(\d+)$/;
const stableTagPattern = /^pixelpitch-v(\d+\.\d+\.\d+)$/;
const betaVersionPattern = /^(\d+\.\d+\.\d+)-beta\.(\d+)$/;
const betaTagPattern = /^pixelpitch-v(\d+\.\d+\.\d+)-beta\.(\d+)(?:\.unsigned)?$/;
...
const versionTag = `pixelpitch-v${betaVersion}${unsignedSuffix}`;
const branch = process.env.GITHUB_REF_NAME ?? "";
const commit = process.env.GITHUB_SHA ?? "";
const releaseName = `Pixelpitch Beta ${betaVersion}${signed ? "" : " (unsigned)"}`;
