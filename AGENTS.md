...
- Architecture and protocols: `docs/spec.md`, `docs/architecture.md`, `docs/skills-protocol.md`, `docs/agent-adapters.md`, `docs/modes.md`, `docs/references.md`.
- Slidify (the Python HTML→PPTX converter): `slidify/`, run `slidify guide` for in-tree long-form guides.
- Attribution: `THIRD_PARTY_NOTICES.md`.
- Directory-level agent guidance: `apps/AGENTS.md`, `packages/AGENTS.md`, `tools/AGENTS.md`.

## Workspace directories

- Workspace packages come from the root `package.json#workspaces`: `apps/*`, `packages/*`, `components`, `tools/dev`, `tools/pack`, `tools/pattern-gen`, `e2e`.
...
- `apps/packaged` is the thin packaged Electron runtime entry; it starts packaged sidecars and owns the `pixelpitch://` entry glue only.
- `packages/contracts` is the pure TypeScript web/daemon app contract layer.
- `packages/sidecar-proto` owns the Pixelpitch sidecar business protocol; `packages/sidecar` owns the generic sidecar runtime; `packages/platform` owns generic OS process primitives.
...
- Runtime target is **Bun `>=1.1`** and **Node `>=22`**; install Bun from https://bun.sh.
...
- One-shot bootstrap: `bun run bootstrap` (installs JS deps, builds the workspace dependency chain, mirrors skills, optionally bootstraps slidify if `uv` is on PATH).
- Start daemon + web for local dev: `bun run dev` (or `make up`).
- Health check: `bun run doctor` (or `make doctor`).
- Stop / status / logs: `bun run stop`, `bun run status`, `bun run logs`.
- The dev runner under `tools/dev/` (renamed from upstream OD's `tools-dev`) is the only sanctioned lifecycle entry point.
- Ports are governed by `tools-dev` flags: `--daemon-port` (default `17456`) and `--web-port` (default `3000`).
- `tools-dev` exports `PIXELPITCH_PORT` for the web proxy target and `PIXELPITCH_WEB_PORT` for the web listener.
...
- Default runtime files live under `<project-root>/.tmp/<source>/<namespace>/...`; POSIX IPC sockets are fixed at `/tmp/pixelpitch/ipc/<namespace>/<app>.sock`.
...
pnpm tools-dev inspect desktop screenshot --path /tmp/pixelpitch.png
pnpm tools-dev stop
...
`@pixelpitch/sidecar-proto` owns Pixelpitch app/mode/source constants, namespace validation, stamp fields/flags, IPC message schema, status shapes, and error semantics. `@pixelpitch/sidecar` provides only generic bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime files. `@pixelpitch/platform` provides only generic OS process stamp serialization, command parsing, and process matching/search primitives, consuming the proto descriptor.

## Where is data written?

The daemon writes `.pixelpitch/` by default: SQLite at `.pixelpitch/app.sqlite`, agent CWDs under `.pixelpitch/projects/<id>/`, and saved renders under `.pixelpitch/artifacts/`. `PIXELPITCH_DATA_DIR` can relocate data relative to the repo root; Playwright uses it to isolate test data.
