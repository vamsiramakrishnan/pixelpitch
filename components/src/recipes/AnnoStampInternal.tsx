...
export default function AnnoStampInternal(props: AnnoStampInternalProps): ReactNode {
  ...
  return (
    <div data-recipe-id="anno.stamp-internal" data-recipe-version="1.0.0">
      <AnnotationBadge {...({ bbox: props.bbox, rotateDeg: props.rotateDeg, kind: 'stamp', tone: 'warn', label: 'INTERNAL' } as unknown as ComponentProps<typeof AnnotationBadge>)} />
    </div>
  );
}
...
export function annoStampInternalToIR(
  ...
  const primitiveArgs = { bbox: props.bbox, rotateDeg: props.rotateDeg, kind: 'stamp', tone: 'warn', label: 'INTERNAL' } as unknown as Parameters<typeof annotationBadgeToIR>[0];
