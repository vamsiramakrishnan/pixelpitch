...
export default function AnnoStampNew(props: AnnoStampNewProps): ReactNode {
  ...
  return (
    <div data-recipe-id="anno.stamp-new" data-recipe-version="1.0.0">
      <AnnotationBadge {...({ bbox: props.bbox, rotateDeg: props.rotateDeg, kind: 'stamp', tone: 'success', label: 'NEW' } as unknown as ComponentProps<typeof AnnotationBadge>)} />
    </div>
  );
}
...
export function annoStampNewToIR(
  ...
  const primitiveArgs = { bbox: props.bbox, rotateDeg: props.rotateDeg, kind: 'stamp', tone: 'success', label: 'NEW' } as unknown as Parameters<typeof annotationBadgeToIR>[0];
