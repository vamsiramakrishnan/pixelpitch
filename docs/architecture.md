...
One `./setup.sh && bun run dev` path installs the workspace, starts the Next.js app, and starts the daemon. Zero config. No accounts.
...
browser ──► od.yourdomain.com (Vercel)
              ...
              ▼
        pixelpitch daemon on user's laptop
              │
              ▼
        spawns: claude / codex / …
```
...
- Listen on `http://localhost:17456` by default. Accept REST/SSE routes under `/api/*`.
...
Rationale:
- **Plain files** → users can `git add ./.pixelpitch/artifacts/` and review designs in PRs.
...
3. Daemon:
     ...
     c. materializes a new artifact dir under ./.pixelpitch/artifacts/<slug>/
...
services:
  daemon:
    ...
    volumes: [ "~/.pixelpitch:/root/.pixelpitch", "./:/workspace" ]
    ports: ["17456:17456"]
  web:
    image: openclaudedesign/web
    ports: ["3000:3000"]
    environment: [ "PIXELPITCH_DAEMON_URL=http://daemon:17456" ]
```
...
# flip VERCEL env flag PIXELPITCH_MODE=direct to hide daemon-connect UI
```
