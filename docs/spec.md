...
- **Artifact store** — project-scoped folder (default `./.pixelpitch/`) holding generated files, version snapshots (git-friendly), and per-artifact metadata.
...
- **Artifact versioning.** Git, or SQLite, or both? [Open CoDesign][ocod] uses SQLite; that's easier but less reviewable. Lean: write artifacts as plain files + a `.pixelpitch/history.jsonl` log. Git is the user's business.
