...
Run all adapters' `detect()` in parallel on daemon start, then cache results in `~/.pixelpitch/agents.json` with a 24h TTL. Re-detect on daemon `SIGHUP`.
